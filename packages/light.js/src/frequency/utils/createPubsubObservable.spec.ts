...
it('should return an Observable', () => {
  setApi(resolveApi());
  expect(
    isObservable(createPubsubObservable('sof_blockNumber', 'sof_blockNumber'))
  ).toBe(true);
});

it('should fire an event when pubsub publishes', done => {
  setApi(resolveApi());
  createPubsubObservable('sof_blockNumber', 'sof_blockNumber').subscribe(
    data => {
...
describe('should manage polling sparingly', () => {
  ...
  const initial$ = createPubsubObservable('sof_blockNumber', 'sof_blockNumber');
  const copy$ = createPubsubObservable('sof_blockNumber', 'sof_blockNumber');
...
describe('should manage pubsub connection sparingly', () => {
  ...
  it('should defer pubsub subscription', async done => {
    obs = createPubsubObservable('sof_blockNumber', 'sof_blockNumber');
  ...
  it('should re-use the same pubsub when used twice', async () => {
    const obs2: Observable<number> = createPubsubObservable('sof_blockNumber', 'sof_blockNumber');
