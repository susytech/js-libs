...
describe('transport/Http', () => {
  ...
  describe('transport emitter', () => {
    it('emits close event', (done) => {
      ...
      transport.execute('sof_call');
    });

    it('emits open event', (done) => {
      mockHttp([{ method: 'sof_call', reply: { result: '' } }]);
  ...
  describe('transport', () => {
    ...
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_call', reply: { result: RESULT } }]);

      return transport
        .execute('sof_call', [1, 2, 3, 'test'])
        .then((_result) => {
          result = _result;
        });
    });
    ...
    it('sets jsonrpc', () => {
      expect(scope.body.sof_call.jsonrpc).to.equal('2.0');
    });

    it('sets the method', () => {
      expect(scope.body.sof_call.method).to.equal('sof_call');
    });

    it('passes the params', () => {
      expect(scope.body.sof_call.params).to.deep.equal([1, 2, 3, 'test']);
    });

    it('increments the id', () => {
      expect(scope.body.sof_call.id).not.to.equal(0);
    });
  ...
  describe('HTTP errors', () => {
    ...
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_call', reply: {}, code: 500 }]);
  ...
  describe('RPC errors', () => {
    ...
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_call', reply: { error: ERROR } }]);
