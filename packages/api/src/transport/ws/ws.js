...
class Ws extends JsonRpcBase {
  ...
  connect () {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      setInterval(() => {
        ...
        if (this._debug) {
          console.log('::susyWS', `speed: ${s} req/s`, `count: ${this._count}`, `(+${n})`);
        }
  ...
  _extract (result) {
    ...
    if (id && msg.subscription && !error) {
      ...
      // resolve promise with messageId because subId's can collide (sof/susy)
      msg.resolve(id);
    ...
    try {
      ...
      if (result.error) {
        ...
        if (!/(rejected|not yet up)/.test(result.error.message)) {
          ...
          const dangerous_methods = ['signer_confirmRequest', 'signer_confirmRequestWithToken', 'susy_exportAccount'];
          let safe_params;
