...
const Abi = require('@susy-js/abi').default;
...
describe('contract/Contract', () => {
  ...
  describe('constructor', () => {
    it('needs an SofAbi instance', () => {
      expect(() => new Contract()).to.throw(/API instance needs to be provided to Contract/);
    });

    it('needs an ABI', () => {
      expect(() => new Contract(sof)).to.throw(/ABI needs to be provided to Contract instance/);
    });

    describe('internal setup', () => {
      const contract = new Contract(sof, ABI);

      it('sets SofApi & parsed interface', () => {
        expect(contract.address).to.not.be.ok;
        expect(contract.api).to.deep.equal(sof);
  ...
  describe('at', () => {
    it('sets returns the functions, events & sets the address', () => {
      const contract = new Contract(sof, [
        {
          constant: true,
          inputs: [{
            name: '_who',
  ...
  describe('_pollTransactionReceipt', () => {
    ...
    const RECSIPT = { contractAddress: ADDRESS.toLowerCase(), blockNumber: BLOCKNUMBER };
    ...
    describe('success', () => {
      before(() => {
        scope = mockHttp([
          { method: 'sof_getTransactionReceipt', reply: { result: null } },
          { method: 'sof_getTransactionReceipt', reply: { result: null } },
          { method: 'sof_getTransactionReceipt', reply: { result: RECSIPT } }
        ]);
      ...
      it('passes the txhash through', () => {
        expect(scope.body.sof_getTransactionReceipt.params[0]).to.equal('0x123');
      });
    ...
    describe('error', () => {
      before(() => {
        scope = mockHttp([{ method: 'sof_getTransactionReceipt', reply: { error: { code: -1, message: 'failure' } } }]);
      });
  ...
  describe('deploy without parameters', () => {
    const contract = new Contract(sof, ABI_NO_PARAMS);
    const CODE = '0x123';
    const ADDRESS = '0xD337e80eEdBdf86eDBba021797d7e4e00Bb78351';
    const RECSIPT_DONE = { contractAddress: ADDRESS.toLowerCase(), gasUsed: 50, blockNumber: 2500 };
    ...
    describe('success', () => {
      before(() => {
        scope = mockHttp([
          { method: 'sof_estimateGas', reply: { result: 1000 } },
          { method: 'susy_postTransaction', reply: { result: '0x678' } },
          { method: 'susy_checkRequest', reply: { result: '0x890' } },
          { method: 'sof_getTransactionReceipt', reply: { result: RECSIPT_DONE } },
          { method: 'sof_getCode', reply: { result: CODE } }
        ]);
      ...
      it('passes the options through to postTransaction (incl. gas calculation)', () => {
        expect(scope.body.susy_postTransaction.params[0].data).to.equal(CODE);
      });
    });
  });
  ...
  describe('deploy', () => {
    const contract = new Contract(sof, ABI);
    const ADDRESS = '0xD337e80eEdBdf86eDBba021797d7e4e00Bb78351';
    const RECSIPT_PEND = { contractAddress: ADDRESS.toLowerCase(), gasUsed: 50, blockNumber: 0 };
    const RECSIPT_DONE = { contractAddress: ADDRESS.toLowerCase(), gasUsed: 50, blockNumber: 2500 };
    const RECSIPT_EXCP = { contractAddress: ADDRESS.toLowerCase(), gasUsed: 1200, blockNumber: 2500 };
    ...
    describe('success', () => {
      before(() => {
        scope = mockHttp([
          { method: 'sof_estimateGas', reply: { result: 1000 } },
          { method: 'susy_postTransaction', reply: { result: '0x678' } },
          { method: 'susy_checkRequest', reply: { result: null } },
          { method: 'susy_checkRequest', reply: { result: '0x890' } },
          { method: 'sof_getTransactionReceipt', reply: { result: null } },
          { method: 'sof_getTransactionReceipt', reply: { result: RECSIPT_PEND } },
          { method: 'sof_getTransactionReceipt', reply: { result: RECSIPT_DONE } },
          { method: 'sof_getCode', reply: { result: '0x456' } }
        ]);
      ...
      it('passes the options through to postTransaction (incl. gas calculation)', () => {
        expect(scope.body.susy_postTransaction.params).to.deep.equal([
    ...
    describe('error', () => {
      it('fails when gasUsed == gas', () => {
        mockHttp([
          { method: 'sof_estimateGas', reply: { result: 1000 } },
          { method: 'susy_postTransaction', reply: { result: '0x678' } },
          { method: 'susy_checkRequest', reply: { result: '0x789' } },
          { method: 'sof_getTransactionReceipt', reply: { result: RECSIPT_EXCP } }
        ]);
      ...
      it('fails when no code was deployed', () => {
        mockHttp([
          ...
          { method: 'susy_checkRequest', reply: { result: '0x789' } },
          { method: 'sof_getTransactionReceipt', reply: { result: RECSIPT_DONE } },
          { method: 'sof_getCode', reply: { result: '0x' } }
        ]);
  ...
  describe('bindings', () => {
    ...
    describe('attachments', () => {
      ...
      it('attaches .call only to constant functions', () => {
        func = (new Contract(sof, [{ type: 'function', name: 'test', constant: true }])).functions[0];
    ...
    describe('postTransaction', () => {
      beforeEach(() => {
        scope = mockHttp([{ method: 'susy_postTransaction', reply: { result: ['hashId'] } }]);
      });

      it('encodes options and mades an susy_postTransaction call', () => {
        return func
          .postTransaction({ someExtras: 'foo' }, VALUES)
          .then(() => {
            expect(scope.isDone()).to.be.true;
            expect(scope.body.susy_postTransaction.params[0]).to.deep.equal({
              someExtras: 'foo',
              to: ADDR,
              data: ENCODED
            });
    ...
    describe('estimateGas', () => {
      ...
      it('encodes options and mades an sof_estimateGas call', () => {
        return func
          .estimateGas({ someExtras: 'foo' }, VALUES)
          .then((amount) => {
            expect(scope.isDone()).to.be.true;
            expect(amount.toString(16)).to.equal('123');
            expect(scope.body.sof_estimateGas.params).to.deep.equal([{
              someExtras: 'foo',
              to: ADDR,
              data: ENCODED
            }]);
    ...
    describe('call', () => {
      it('encodes options and mades an sof_call call', () => {
        scope = mockHttp([{ method: 'sof_call', reply: { result: RETURN1 } }]);

        return func
          .call({ someExtras: 'foo' }, VALUES)
          .then((result) => {
            expect(scope.isDone()).to.be.true;
            expect(scope.body.sof_call.params).to.deep.equal([{
              someExtras: 'foo',
              to: ADDR,
              data: ENCODED
            }, 'latest']);
      ...
      it('encodes options and mades an sof_call call (multiple returns)', () => {
        scope = mockHttp([{ method: 'sof_call', reply: { result: `${RETURN1}${RETURN2}` } }]);
  ...
  describe('subscribe', () => {
    ...
    describe('valid events', () => {
      ...
      beforeEach(() => {
        scope = mockHttp([
          { method: 'sof_newFilter', reply: { result: '0x123' } },
          { method: 'sof_getFilterLogs', reply: { result: logs } },
          { method: 'sof_getFilterChanges', reply: { result: logs } },
          { method: 'sof_newFilter', reply: { result: '0x123' } },
          { method: 'sof_getFilterLogs', reply: { result: logs } }
        ]);
        cbb = sinon.stub();
