...
class Contract {
  ...
  deployEstimateGas(options, values) {
    ...
    return this._api.sof.estimateGas(_options).then(gasEst => {
    ...
    return gasEstPromise.then(_gas => {
      ...
      return this._api.susy
        ...
        .then(address => {
          ...
          return this._api.sof.getCode(this._address);
        })
        .then(code => {
          if (code === '0x') {
  ...
  _pollCheckRequest(requestId) {
    return this._api.pollMethod('susy_checkRequest', requestId);
  }

  _pollTransactionReceipt(txhash, gas) {
    return this.api.pollMethod('sof_getTransactionReceipt', txhash, receipt => {
  ...
  _bindFunction(func) {
    ...
    if (!func.constant) {
      func.postTransaction = (options, values = []) => {
        ...
        return this._api.susy.postTransaction(_options).catch(error => {
      ...
      func.estimateGas = (options, values = []) => {
        ...
        return this._api.sof.estimateGas(_options).catch(error => {
  ...
  _createSofFilter(event = null, _options) {
    const options = this._getFilterOptions(event, _options);

    return this._api.sof.newFilter(options);
  }
  ...
  _subscribe(event = null, _options, callback, autoRemove = false) {
    ...
    return this._createSofFilter(event, _options)
      .then(filterId => {
        ...
        return this._api.sof.getFilterLogs(filterId).then(logs => {
  ...
  _subscribeToBlock() {
    this._api
      .subscribe('sof_blockNumber', error => {
        if (error) {
  ...
  _sendSubscriptionChanges(subscriptions) {
    return Promise.all(
      subscriptions.map(subscription => {
        return this._api.sof.getFilterChanges(subscription.filterId);
      })
    )
      .then(logsArray => {
