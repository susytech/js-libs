...
describe('subscriptions/personal', () => {
  ...
  describe('start', () => {
    describe('info available', () => {
      ...
      it('calls susy_accountsInfo', () => {
        expect(api._calls.accountsInfo.length).to.be.ok;
      });

      it('calls susy_allAccountsInfo', () => {
        expect(api._calls.allAccountsInfo.length).to.be.ok;
      });

      it('calls sof_accounts', () => {
        expect(api._calls.listAccounts.length).to.be.ok;
      });

      it('updates subscribers', () => {
        expect(cb).to.have.been.calledWith('susy_defaultAccount', null, TEST_DEFAULT);
        expect(cb).to.have.been.calledWith('sof_accounts', null, TEST_LIST);
        expect(cb).to.have.been.calledWith('susy_accountsInfo', null, TEST_INFO);
        expect(cb).to.have.been.calledWith('susy_allAccountsInfo', null, TEST_INFO);
      });
    });
