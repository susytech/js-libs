...
const instance = new Sof(new PromiseProvider(new Http(TEST_HTTP_URL, -1)));
...
describe('rpc/Sof', () => {
  ...
  describe('accounts', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_accounts', reply: { result: [address.toLowerCase()] } }]);
    });
  ...
  describe('blockNumber', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_blockNumber', reply: { result: '0x123456' } }]);
    });
  ...
  describe('call', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_call', reply: { result: [] } }]);
    });

    it('formats the input options & blockNumber', () => {
      return instance.call({ data: '12345678' }, 'earliest').then(() => {
        expect(scope.body.sof_call.params).to.deep.equal([{ data: '0x12345678' }, 'earliest']);
      });
    });

    it('provides a latest blockNumber when not specified', () => {
      return instance.call({ data: '12345678' }).then(() => {
        expect(scope.body.sof_call.params).to.deep.equal([{ data: '0x12345678' }, 'latest']);
      });
    });
  });
  ...
  describe('coinbase', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_coinbase', reply: { result: address.toLowerCase() } }]);
    });
  ...
  ['LLL', 'Serpent', 'Solidity'].forEach((type) => {
    ...
    describe(method, () => {
      beforeEach(() => {
        scope = mockHttp([{ method: `sof_${method}`, reply: { result: '0x123' } }]);
      });

      it('formats the input as data, returns the output', () => {
        return instance[method]('0xabcdef').then((result) => {
          expect(scope.body[`sof_${method}`].params).to.deep.equal(['0xabcdef']);
  ...
  describe('estimateGas', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_estimateGas', reply: { result: '0x123' } }]);
    });

    it('converts the options correctly', () => {
      return instance.estimateGas({ gas: 21000 }).then(() => {
        expect(scope.body.sof_estimateGas.params).to.deep.equal([{ gas: '0x5208' }]);
      });
    });
  ...
  describe('gasPrice', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_gasPrice', reply: { result: '0x123' } }]);
    });
  ...
  describe('getBalance', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getBalance', reply: { result: '0x123' } }]);
    });

    it('passes in the address (default blockNumber)', () => {
      return instance.getBalance(address).then(() => {
        expect(scope.body.sof_getBalance.params).to.deep.equal([address.toLowerCase(), 'latest']);
      });
    });

    it('passes in the address & blockNumber', () => {
      return instance.getBalance(address, 0x456).then(() => {
        expect(scope.body.sof_getBalance.params).to.deep.equal([address.toLowerCase(), '0x456']);
      });
    });
  ...
  describe('getBlockByHash', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getBlockByHash', reply: { result: { miner: address.toLowerCase() } } }]);
    });

    it('formats the input hash as a hash, default full', () => {
      return instance.getBlockByHash('1234').then(() => {
        expect(scope.body.sof_getBlockByHash.params).to.deep.equal(['0x1234', false]);
      });
    });

    it('formats the input hash as a hash, full true', () => {
      return instance.getBlockByHash('1234', true).then(() => {
        expect(scope.body.sof_getBlockByHash.params).to.deep.equal(['0x1234', true]);
      });
    });
  ...
  describe('getBlockByNumber', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getBlockByNumber', reply: { result: { miner: address.toLowerCase() } } }]);
    });

    it('assumes blockNumber latest & full false', () => {
      return instance.getBlockByNumber().then(() => {
        expect(scope.body.sof_getBlockByNumber.params).to.deep.equal(['latest', false]);
      });
    });

    it('uses input blockNumber & full false', () => {
      return instance.getBlockByNumber('0x1234').then(() => {
        expect(scope.body.sof_getBlockByNumber.params).to.deep.equal(['0x1234', false]);
      });
    });

    it('formats the input blockNumber, full true', () => {
      return instance.getBlockByNumber(0x1234, true).then(() => {
        expect(scope.body.sof_getBlockByNumber.params).to.deep.equal(['0x1234', true]);
      });
    });
  ...
  describe('getBlockTransactionCountByHash', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getBlockTransactionCountByHash', reply: { result: '0x123' } }]);
    });

    it('formats input hash properly', () => {
      return instance.getBlockTransactionCountByHash('abcdef').then(() => {
        expect(scope.body.sof_getBlockTransactionCountByHash.params).to.deep.equal(['0xabcdef']);
      });
    });
  ...
  describe('getBlockTransactionCountByNumber', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getBlockTransactionCountByNumber', reply: { result: '0x123' } }]);
    });

    it('specified blockNumber latest when none specified', () => {
      return instance.getBlockTransactionCountByNumber().then(() => {
        expect(scope.body.sof_getBlockTransactionCountByNumber.params).to.deep.equal(['latest']);
      });
    });

    it('formats input blockNumber properly', () => {
      return instance.getBlockTransactionCountByNumber(0xabcdef).then(() => {
        expect(scope.body.sof_getBlockTransactionCountByNumber.params).to.deep.equal(['0xabcdef']);
      });
    });
  ...
  describe('getCode', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getCode', reply: { result: '0x1234567890' } }]);
    });

    it('passes in the address (default blockNumber)', () => {
      return instance.getCode(address).then(() => {
        expect(scope.body.sof_getCode.params).to.deep.equal([address.toLowerCase(), 'latest']);
      });
    });

    it('passes in the address & blockNumber', () => {
      return instance.getCode(address, 0x456).then(() => {
        expect(scope.body.sof_getCode.params).to.deep.equal([address.toLowerCase(), '0x456']);
      });
    });
  ...
  describe('getStorageAt', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getStorageAt', reply: { result: '0x1234567890' } }]);
    });

    it('passes in the address (default index& blockNumber)', () => {
      return instance.getStorageAt(address).then(() => {
        expect(scope.body.sof_getStorageAt.params).to.deep.equal([address.toLowerCase(), '0x0', 'latest']);
      });
    });

    it('passes in the address, index & blockNumber', () => {
      return instance.getStorageAt(address, 15, 0x456).then(() => {
        expect(scope.body.sof_getStorageAt.params).to.deep.equal([address.toLowerCase(), '0xf', '0x456']);
      });
    });
  ...
  describe('getTransactionByBlockHashAndIndex', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getTransactionByBlockHashAndIndex', reply: { result: { to: address.toLowerCase() } } }]);
    });

    it('passes in the hash (default index)', () => {
      return instance.getTransactionByBlockHashAndIndex('12345').then(() => {
        expect(scope.body.sof_getTransactionByBlockHashAndIndex.params).to.deep.equal(['0x12345', '0x0']);
      });
    });

    it('passes in the hash & specified index', () => {
      return instance.getTransactionByBlockHashAndIndex('6789', 0x456).then(() => {
        expect(scope.body.sof_getTransactionByBlockHashAndIndex.params).to.deep.equal(['0x6789', '0x456']);
      });
    });
  ...
  describe('getTransactionByBlockNumberAndIndex', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getTransactionByBlockNumberAndIndex', reply: { result: { to: address.toLowerCase() } } }]);
    });

    it('passes in the default parameters', () => {
      return instance.getTransactionByBlockNumberAndIndex().then(() => {
        expect(scope.body.sof_getTransactionByBlockNumberAndIndex.params).to.deep.equal(['latest', '0x0']);
      });
    });

    it('passes in the blockNumber & specified index', () => {
      return instance.getTransactionByBlockNumberAndIndex('0x6789', 0x456).then(() => {
        expect(scope.body.sof_getTransactionByBlockNumberAndIndex.params).to.deep.equal(['0x6789', '0x456']);
      });
    });
  ...
  describe('getTransactionByHash', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getTransactionByHash', reply: { result: { to: address.toLowerCase() } } }]);
    });

    it('passes in the hash', () => {
      return instance.getTransactionByHash('12345').then(() => {
        expect(scope.body.sof_getTransactionByHash.params).to.deep.equal(['0x12345']);
      });
    });
  ...
  describe('getTransactionCount', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getTransactionCount', reply: { result: '0x123' } }]);
    });

    it('passes in the address (default blockNumber)', () => {
      return instance.getTransactionCount(address).then(() => {
        expect(scope.body.sof_getTransactionCount.params).to.deep.equal([address.toLowerCase(), 'latest']);
      });
    });

    it('passes in the address & blockNumber', () => {
      return instance.getTransactionCount(address, 0x456).then(() => {
        expect(scope.body.sof_getTransactionCount.params).to.deep.equal([address.toLowerCase(), '0x456']);
      });
    });
  ...
  describe('getUncleByBlockHashAndIndex', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getUncleByBlockHashAndIndex', reply: { result: [] } }]);
    });

    it('passes in the hash (default index)', () => {
      return instance.getUncleByBlockHashAndIndex('12345').then(() => {
        expect(scope.body.sof_getUncleByBlockHashAndIndex.params).to.deep.equal(['0x12345', '0x0']);
      });
    });

    it('passes in the hash & specified index', () => {
      return instance.getUncleByBlockHashAndIndex('6789', 0x456).then(() => {
        expect(scope.body.sof_getUncleByBlockHashAndIndex.params).to.deep.equal(['0x6789', '0x456']);
      });
    });
  });
  ...
  describe('getUncleByBlockNumberAndIndex', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getUncleByBlockNumberAndIndex', reply: { result: [] } }]);
    });

    it('passes in the default parameters', () => {
      return instance.getUncleByBlockNumberAndIndex().then(() => {
        expect(scope.body.sof_getUncleByBlockNumberAndIndex.params).to.deep.equal(['latest', '0x0']);
      });
    });

    it('passes in the blockNumber & specified index', () => {
      return instance.getUncleByBlockNumberAndIndex('0x6789', 0x456).then(() => {
        expect(scope.body.sof_getUncleByBlockNumberAndIndex.params).to.deep.equal(['0x6789', '0x456']);
      });
    });
  });
  ...
  describe('getUncleCountByBlockHash', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getUncleCountByBlockHash', reply: { result: '0x123' } }]);
    });

    it('passes in the hash', () => {
      return instance.getUncleCountByBlockHash('12345').then(() => {
        expect(scope.body.sof_getUncleCountByBlockHash.params).to.deep.equal(['0x12345']);
      });
    });
  ...
  describe('getUncleCountByBlockNumber', () => {
    beforeEach(() => {
      scope = mockHttp([{ method: 'sof_getUncleCountByBlockNumber', reply: { result: '0x123' } }]);
    });

    it('passes in the default parameters', () => {
      return instance.getUncleCountByBlockNumber().then(() => {
        expect(scope.body.sof_getUncleCountByBlockNumber.params).to.deep.equal(['latest']);
      });
    });

    it('passes in the blockNumber', () => {
      return instance.getUncleCountByBlockNumber('0x6789').then(() => {
        expect(scope.body.sof_getUncleCountByBlockNumber.params).to.deep.equal(['0x6789']);
      });
    });
